...
<!-- line break in output file; improves human readability of xml output -->
    ...
    <xsl:template match="/">
        ...
        <html>
            <head>
                <title>Atalanta Fugiens (Beinecke Ms. transcription) Normalized Reading Copy. Emblem <xsl:value-of select="af:div[@type='emblem/@n']"/></title>
                <link rel="stylesheet" type="text/css" href="atalantaProof-color-norm.css"  />
            </head>
    ...
    <xsl:template match="af:pb"><span class="pb"><xsl:value-of select="@n"/></span>
    </xsl:template>
    
    <xsl:template match="af:lb">
       <xsl:choose>
           <xsl:when test="not(@break='no')"><xsl:text> </xsl:text></xsl:when>
       </xsl:choose>
    </xsl:template>
    
    <xsl:template match="af:ab//text()">
        <!--<xsl:variable name="me" select="normalize-space(.)"/>-->
        <xsl:choose>
            <xsl:when test="following-sibling::*[1][self::af:lb[@break eq 'no']]">
    ...
    <!-- choice/orig/reg -->
    <xsl:template match="af:reg"><span class="{name()}"><xsl:apply-templates/></span></xsl:template>
    
    <xsl:template match="af:orig">      
        <xsl:choose>
            <xsl:when test="not(parent::af:choice)"><span class="orig-solo"><xsl:apply-templates/></span></xsl:when>
        </xsl:choose>      
    </xsl:template>
